...
/*   Updated: 2022/10/28 20:09:13 by ateak            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parser(t_info *data, char **argv, int fd)
{
	...
	map = NULL;
	i = 0;
	get_map_data(&map, argv, fd); //сохраняем карту в массив используя гнл
	i = parse_map_data(&map, data, i); // Сохраняет все аргументы в нужные переменные структуры и возвращает номер строки где начинается карта
	check_player_find_map_size(map, data, i); //проверка валидности карты, сохранение позиции игрока камеры и размера карты
	save_map_into_array(map, data, i); //выравниваем карту до прямогуольной и сохраняем в массив glb->map.map_array
